...
from MsneddonContigFilter2.MsneddonContigFilter2Impl import MsneddonContigFilter2


class MsneddonContigFilter2Test(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'MsneddonContigFilter2',
        ...
        for nameval in config.items('MsneddonContigFilter2'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = MsneddonContigFilter2(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_MsneddonContigFilter2_" + str(suffix)
